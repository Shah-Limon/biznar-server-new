...
async function run() {
  try {
    ...
    const CTAOptionCollections = client
      .db("seoWebsite")
      .collection("CTAOption");
    ...
    app.put("/edit-speciality/:id", async (req, res) => {
      ...
      const updatedDoc = {
        $set: {
          ...
          featureOne: updateSpeciality.featureOne,
          featureTwo: updateSpeciality.featureTwo,
          featureThree: updateSpeciality.featureThree,
          featureFour: updateSpeciality.featureFour,
        },
      };
    ...
    app.get("/all-cta", async (req, res) => {
      const query = {};
      const cursor = CTAOptionCollections.find(query);
      const choose = await cursor.toArray();
      res.send(choose);
    });
    app.get("/cta/:id", async (req, res) => {
      const query = {};
      const cursor = CTAOptionCollections.find(query);
      const choose = await cursor.toArray();
      res.send(choose);
    ...
    app.put("/edit-cta/:id", async (req, res) => {
      const id = req.params.id;
      const updateChoose = req.body;
      const filter = { _id: new ObjectId(id) };
      const options = { upsert: true };
      const updatedDoc = {
        $set: {
          ...
          desc: updateChoose.desc,
          youtubeLink: updateChoose.youtubeLink,
          buttonText: updateChoose.buttonText,
          buttonLink: updateChoose.buttonLink,
        },
      };

      const result = await CTAOptionCollections.updateOne(
        filter,
        updatedDoc,
        options
      );
